...
WEKA_EVAL=""
WEKA_SEARCH= "Ranker" 
N=11 #11 #11 #11 #11 #11 #11 #11 #11 #11 #11 #20
D=1
...
if [[ ${CLASSIFIER} == "NaiveBayes" ]]; then
    ...
    WEKA_CLS_TRAIN="weka.classifiers.functions.SMO -C 0.125 -K weka.classifiers.functions.supportVector.RBFKernel -G 0.25"
...
if [ ! -f ${MODEL_PATH}/${MODEL_FILE}.model ]; then
  java -classpath ../../../weka-3-8-0/weka.jar:/home/intel_parking/wekafiles/packages/LibSVM1.0.10/LibSVM.jar:/home/intel_parking/wekafiles/packages/LibSVM1.0.10/lib/libsvm.jar ${WEKA_CLS_TRAIN} -t ${DATA_PATH}/${TRAIN_FILE2}.arff.gz -d ${MODEL_PATH}/${MODEL_FILE}.model -no-cv -o
fi


## Testing
echo "Testing: -l ${MODEL_PATH}/${MODEL_FILE}.model -T ${DATA_PATH}/${TEST_FILE2}.arff > ${PRED_PATH}/${OUTPUT_FILE}.pred.csv"
java -classpath ../../../weka-3-8-0/weka.jar:/home/intel_parking/wekafiles/packages/LibSVM1.0.10/LibSVM.jar:/home/intel_parking/wekafiles/packages/LibSVM1.0.10/lib/libsvm.jar ${WEKA_CLS_TEST} -l ${MODEL_PATH}/${MODEL_FILE}.model -T ${DATA_PATH}/${TEST_FILE2}.arff.gz -classifications "weka.classifiers.evaluation.output.prediction.CSV" > ${PRED_PATH}/${OUTPUT_FILE}.pred.csv
