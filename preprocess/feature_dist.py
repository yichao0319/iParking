...
##
## - Example
##   python feature_dist.py weka_4.norm.fix 1
##
######################################
...
## =====================
if len(sys.argv) < 2:
  print "FORMAT: filename [if_compare]"
  exit()

filename = sys.argv[1]
if_compare = 0
if len(sys.argv) == 3:
  if_compare = int(sys.argv[2])
...
f = open("%s%s.dist.txt" % (output_dir, filename), 'w')
...
for fi in range(nfig):
  ...
  for si in range(3):
    ...
    plt.savefig("%s%s.dist.s%d.months.%d.eps" % (fig_dir, filename, si, feature_idx), format='eps', dpi=1000)
